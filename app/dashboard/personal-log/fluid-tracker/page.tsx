...
const goalSlices = [
  { label: "Goal Met", count: 29, percent: 50, color: "#3B82F6" },
  { label: "Above Goal", count: 16, percent: 28, color: "#F59E0B" },
  { label: "Below Goal", count: 13, percent: 22, color: "#FF5630" },
];

const fluidIntakeBars = [72, 67, 31, 73, 59, 31, 91, 31, 57, 81, 52, 56, 71, 57, 33];

const urineTrends = [
  { date: "May 10, 2024", trend: "Increasing", icon: "up" as const },
  { date: "May 31, 2024", trend: "Decreasing", icon: "down" as const },
  { date: "Jun 7, 2024", trend: "Increasing", icon: "up" as const },
  { date: "Jul 5, 2024", trend: "No Change", icon: "level" as const },
];

const alerts = [
  {
    title: "Weight Gain Notice",
    body: "+2.6 lbs this month. Possible fluid retention — contact your nurse if swelling increases.",
    tone: "warning",
  },
  {
    title: "Missed Logging",
    body: "3 days without logging detected. Consistent tracking helps your care team.",
    tone: "error",
  },
  {
    title: "Fluid Goal On Track",
    body: "Average 42 oz/day, within your 48 oz limit. Keep it up.",
    tone: "success",
  },
  {
    title: "Education Tip",
    body: 'Watch "Understanding Fluid Retention" module to learn practical daily tips.',
    tone: "info",
  },
];
      ...
      {metricCards.map((card) => (
        ...
        >
          ...
          <div className="mt-1 flex items-center justify-between gap-2 text-xs">
            <span className={card.noteClass ?? "text-slate-500"}>{card.note}</span>
            {card.extra && <span className="font-medium text-slate-700">{card.extra}</span>}
          </div>
        </article>
      ))}
    </section>
...
function WeightTrendChart() {
  ...
  const points = weightTrend
    ...
    .join(" ");
  const labels = ["May 1", "May 2", "May 3", "May 4", "May 5", "May 6"];
  ...
  return (
    <section className="h-full rounded-xl border border-[#DFE3E8] bg-[#FCFDFD] p-3.5">
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-xs text-slate-600">(30 Day)</span>
      </div>
      <svg viewBox={`0 0 ${width} ${height + 22}`} className="mt-3 h-[236px] w-full">
        {[8, 6, 4, 2, 0].map((tick, index) => {
          const y = top + (index / 4) * plotHeight;
          return (
            <g key={tick}>
              <text x={16} y={y + 4} textAnchor="end" className="fill-black/70 text-[12px]">
                {tick}
              </text>
              <line
                x1={left}
        ...
        <polyline fill="none" stroke="#8979FF" strokeWidth="2.5" points={points} />
      ...
      <div className="mt-4 flex flex-1 flex-col items-center gap-4 sm:flex-row">
        <div className="relative size-[226px] shrink-0">
          <svg viewBox="0 0 226 226" className="size-full -rotate-90">
            <circle cx="113" cy="113" r={radius} fill="none" stroke="#E2E8F0" strokeWidth="28" />
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <p className="text-sm text-[#454F5B]">Overall</p>
          </div>
        </div>
      ...
      <div className="mt-3 flex min-h-[250px] gap-2">
        ...
        <div className="relative min-w-0 flex-1">
          ...
          <div className="absolute inset-x-0 bottom-0 flex justify-between pl-2 text-[12px] text-black/70">
            {["May 1", "May 2", "May 3", "May 4", "May 5", "May 6"].map((label) => (
    ...
    <section className="flex h-full flex-col rounded-xl border border-[#E3E6F0] bg-white p-[17px]">
      ...
      <div className="mt-3.5 overflow-hidden rounded-lg border border-[#C4CDD5]">
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {urineTrends.map((row) => (
              <tr key={row.date} className="border-b border-dashed border-[#C4CDD5] last:border-b-0">
                <td className="px-3 py-3 text-[#1C252E]">{row.date}</td>
                <td className="px-3 py-3">
                  <div className="flex items-center gap-3">
                    <TrendIcon type={row.icon} />
                    <span className="font-medium text-[#1C252E]">{row.trend}</span>
                  </div>
                </td>
              </tr>
            ))}
...
function AlertsInsights() {
  ...
  return (
    <section className="h-full rounded-xl border border-slate-200 bg-white p-4">
      <h2 className="text-sm font-medium text-slate-950">Alerts & Insights</h2>
      <div className="mt-3 space-y-2">
        {alerts.map((alert) => (
          <article
            key={alert.title}
            className={`rounded-lg border p-2.5 ${toneClass[alert.tone as keyof typeof toneClass]}`}
          >
            <div className="flex gap-2">
              ...
              <div>
                <p className="text-xs font-medium text-slate-950">{alert.title}</p>
                <p className="mt-0.5 text-[11px] leading-[13px] text-slate-600">{alert.body}</p>
              </div>
            </div>
          </article>
        ))}
          ...
          <table className="min-w-[1080px] w-full text-left text-sm">
            ...
            <tbody>
              {recentEntries.map((entry) => (
                <tr key={entry.date} className="border-b border-dashed border-[#C4CDD5] last:border-b-0">
                  <td className="px-3 py-3 text-[#1C252E]">{entry.date}</td>
                  ...
                  <td className="px-3 py-3">
                    <GoalBadge status={entry.goal} />
                  </td>
                  <td className="px-3 py-3 text-[#1C252E]">{entry.swelling}</td>
                  <td className="px-3 py-3 text-[#1C252E]">{entry.sob}</td>
                  <td className="px-3 py-3 text-[#1C252E]">{entry.weakness}</td>
                  <td className="px-3 py-3 text-[#1C252E]">{entry.notes}</td>
                </tr>
              ))}
            </tbody>
          </table>
    ...
    <div className="space-y-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-[28px] font-medium leading-none text-slate-950 sm:text-[32px]">
          Weight & Fluid Management Center
        </h1>
        <button
          type="button"
