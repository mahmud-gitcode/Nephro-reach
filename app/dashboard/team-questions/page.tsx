...
export default function TeamQuestionsPage() {
  ...
  // Segmented Pill Tabs: All, Provider, Dietitian, Social Worker, Nurse (No icon, No number tag)
  const roleTabs: {
    id: CareTeamRole | "All";
    label: string;
  }[] = [
    ...
    <div className="w-full max-w-5xl mx-auto pb-12 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-xl bg-[#2563EB] hover:bg-blue-700 px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:shadow transition-all active:scale-[0.98] cursor-pointer self-start sm:self-center"
        >
      ...
      {/* SEGMENTED TAB BAR (All, Provider, Dietitian, Social Worker, Nurse - no icon, no number tag) */}
      ...
      {/* Clean Q & Ans Card List (No Table, No Date) */}
      <div className="space-y-3.5">
        {filteredQuestions.length === 0 ? (
          ...
          filteredQuestions.map((q) => (
            ...
            >
              {/* Question Row with Edit Icon right beside Q, and Status Tag (No Date) */}
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                <div className="flex items-start gap-2.5 flex-1">
                  ...
                  {/* Edit icon beside this question */}
                  <button
                    type="button"
                    onClick={() => handleOpenEditModal(q)}
                    className="p-1 rounded-md text-slate-400 hover:text-blue-600 hover:bg-blue-50 transition-colors cursor-pointer shrink-0 mt-0.5"
                    title={
                      language === "ES"
