          ...
          <TableBody>
            {upcomingClasses.map((item) => (
              <TableRow key={item.date}>
                ...
                <TableCell>
                  <Badge tone={registrationTone[item.status]}>
                    {item.status}
                  </Badge>
                </TableCell>
                <TableCell>
...
function LiveClassCalendar() {
  ...
  const selected = day === null ? [] : classesOn(year, month, day);
  ...
  return (
    <Card as="section" padding="small" className="h-full">
      ...
      <MonthCalendar
        ...
        isMarked={(date) =>
          classesOn(date.getFullYear(), date.getMonth(), date.getDate())
            .length > 0
        }
      />
      ...
      >
        {day === null ? (
          ...
          selected.map((item) => (
            <div key={item.date} className="flex items-start gap-inline-lg">
              <span
                aria-hidden="true"
              ...
              <Badge tone={registrationTone[item.status]}>{item.status}</Badge>
            </div>
          ))
        )}
      </div>
...
function RecentLiveClasses() {
  return (
    <Card as="section" padding="small">
      ...
      <Button
        {...notBuiltYet("The recordings library")}
        variant="neutral"
        appearance="stroke"
        size="small"
        fullWidth
